...
function generateCirclesFun() {
  ...
  for (let i = 0; i < amountCircles; i++) {
    ...
    let randX;
    let randY;
    let tendency = Math.sqrt(
      (constHeight * constWidth) / (amountCircles * Math.PI)
    );
    do {
      ...
      for (let j = 0; j < circles.length; j++) {
        let distance = Math.sqrt(
          Math.pow(circles[j].getCenterX() - randX, 2) +
            Math.pow(circles[j].getCenterY() - randY, 2)
        );
    ...
    let circle = new Circle(randX, randY, 0, DEFAULT_PXL_COLOR);
    circles.push(circle);
  }
...
let setPixelInArrayAndCanvas = function(x, y, pixelState, color, circle) {
  ...
  if (x >= 0 && x < constWidth && y >= 0 && y < constHeight) {
    ...
    if (
      (pxl.getState() == pixelState || pxl.getState() == pixelState - 1) &&
      pxl.getCircle() !== circle
    ) {
